...
char sim_var[] = "lambda";  		// Notes if a variable is varied over runs
...
int main() {	
    ...
    for(Lambda=0.000; Lambda<0.02; Lambda+=0.0025) {
...
/** Adaptivity */
event adapt(i++) {
    adapt_wavelet((scalar *){fan, u,b},(double []){0.01,eps,eps,eps,0.35*9.81/273},maxlevel,minlevel);
}
