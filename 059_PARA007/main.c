...
char sim_var[] = "N";  		// Notes if a variable is varied over runs
...
int main() {	
    ...
    for(tempVar=0.1; tempVar<2.; tempVar*=2.) {
...
event init(t=0) {
    ...
    while(adapt_wavelet((scalar *){u,b},(double []){eps,eps,eps,tempVar*9.81/273},maxlevel,minlevel).nf) {
	foreach() {
	    b[] = STRAT(y);
            u.x[] = WIND(y);
	}
...
/** Adaptivity */
event adapt(i++) {
    adapt_wavelet((scalar *){fan, u,b},(double []){0.01,eps,eps,eps,tempVar*9.81/273},maxlevel,minlevel);
}
